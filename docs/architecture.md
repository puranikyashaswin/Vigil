...
| Step | Description |
|:---|:---|
| Type Detection | Examines file extension and MIME type; routes text-native PDFs/DOCX to local parsers, images/scanned PDFs to OpenRouter OCR |
| Local Parsers | `pdfplumber` (text PDFs), `python-docx`, `openpyxl`/`xlrd` (spreadsheets); structured table extraction into Markdown |
| OpenRouter OCR | Free-tier vision models (`nvidia/nemotron-nano-12b-v2-vl:free`) with automatic fallback chain (`google/gemma-4-26b-a4b-it:free`, `openrouter/free`) |
...
| Dashboard | Next.js with `react-force-graph-2d` (Obsidian-style layout), multi-tab inspector/chat/alerts panel, warm ivory editorial theme |
