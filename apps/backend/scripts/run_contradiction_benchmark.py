...
def run_benchmark():
    ...
    for idx, case in enumerate(benchmark_cases):
        ...
        logger.info(
            f"[{idx+1}/{len(benchmark_cases)}] Testing Pair {pair_id}: {ent_a['name']} vs {ent_b['name']}"
        )
    ...
    contradictory_scores = [
        r["confidence_raw"] for r in cached_results if r["expected"]
    ]
    clean_scores = [r["confidence_raw"] for r in cached_results if not r["expected"]]

    avg_contradictory = (
        sum(contradictory_scores) / len(contradictory_scores)
    ...
    report_content.append("| :--- | :---: | :---: | :---: |")
    report_content.append(
        f"| **Contradictory Pairs** | {len(contradictory_scores)} | {avg_contradictory:.4f} | {med_contradictory:.4f} |"
    )
    report_content.append(
        f"| **Clean Control Pairs** | {len(clean_scores)} | {avg_clean:.4f} | {med_clean:.4f} |"
    )
    ...
    logger.info(
        f"Contradiction benchmark evaluation complete. Report saved to: {report_path}"
    )
    print("\nBenchmark Sweep Results:")
    print("Threshold | Precision | Recall | F1-Score")
    for sr in sweep_results:
        print(
            f"  {sr['threshold']:.1f}     |  {sr['precision']:.4f}   | {sr['recall']:.4f} | {sr['f1']:.4f}"
        )
    print(
