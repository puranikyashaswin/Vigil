            ...
            if file.endswith(".md") and file not in ["index.md", "log.md"]:
                ...
                for link in links:
                    ...
                    if not os.path.exists(target_path):
                        broken_links += 1
                        logger.error(
                            f"[LINK INTEGRITY ERROR] Broken link in {os.path.relpath(file_path, kg_dir)}: Link to '{link}' (Resolved: '{os.path.relpath(target_path, kg_dir)}') does not exist."
                        )

    if broken_links > 0:
        logger.warning(
            f"Link Integrity Audit finished: checked {checked_links} links, found {broken_links} broken link(s)."
        )
    else:
        logger.info(
            f"Link Integrity Audit complete: all {checked_links} relative links are valid!"
        )
...
def main():
    ...
    for ent in all_entities:
        ...
        append_to_log(
            dir_full_path, "INGEST", f"Ingested entity {title} from source {resource}"
        )
        append_to_log(
            kg_dir, "INGEST", f"Ingested entity {title} into category {sub_dir}"
        )
            ...
            for label, target in matches:
                if target.startswith(".") or target.startswith(".."):
                    if label.lower() not in [
                        lc.lower() for lc in ent.get("linked_concepts", [])
                    ]:
    ...
    logger.info(
        f"Identified {len(pairs_to_check)} candidate pairs for contradiction audit."
    )
    ...
    for (ent, target_ent), res in zip(pairs_to_check, results):
        if res.get("contradiction_detected") and res.get("confidence_score", 0) > 0.7:
            ...
            logger.warning(
                f"[CONTRADICTION DETECTED] Severity: {severity}, Confidence: {conf_score}"
            )
            ...
            with open(
                os.path.join(kg_dir, alert_rel_path), "w", encoding="utf-8"
            ) as out_f:
                out_f.write("\n".join(alert_content) + "\n")

            alert_title = f"Conflict: {ent['name']} and {target_ent['name']}"
            append_to_index(
                os.path.join(kg_dir, "alerts"), alert_filename, alert_title, explanation
            )
            append_to_log(
                os.path.join(kg_dir, "alerts"),
                "ALERT",
                f"Detected contradiction between {ent['name']} and {target_ent['name']}",
            )
            append_to_log(
                kg_dir, "ALERT", f"Contradiction alert created: {alert_filename}"
            )

    check_link_integrity(kg_dir)
    logger.info(
        f"Graph construction and safety audit complete. Found {contradiction_count} contradictions."
    )
