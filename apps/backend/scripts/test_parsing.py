...
def parse_args():
    parser = argparse.ArgumentParser(description="Vigil Document Ingestion Parser Test Suite")
    parser.add_argument(
        "--input-dir",
...
def main():
    ...
    for filename in sorted(files):
        ...
        try:
            ...
            if category == "image" or category == "scanned_pdf":
                ...
                parsed_text, model_used = parsers.parse_via_openrouter_ocr(file_path, api_key)
            ...
            status = "PASS"
            logger.info(f"Successfully parsed {filename}. Output written to {output_file}")
    ...
    print("="*80)
    print(f"{'File Name':<35} | {'Route':<20} | {'Status':<6} | {'Time (s)':<8} | {'Details / Error':<20}")
    print("-"*110)
    for res in results:
        details_col = res["details"]
        if len(details_col) > 40:
            details_col = details_col[:37] + "..."
        print(f"{res['file'][:35]:<35} | {res['route']:<20} | {res['status']:<6} | {res['time']:<8.2f} | {details_col}")
    print("="*80 + "\n")
