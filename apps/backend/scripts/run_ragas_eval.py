...
def main():
    ...
    if use_clean_log:
        ...
        logger.info(
            f"Split: {len(in_scope_cases)} In-Scope cases, {len(out_of_scope_cases)} Out-of-Scope refusal cases."
        )
        ...
        for i, item in enumerate(out_of_scope_cases):
            query = item["question"]
            logger.info(
                f"[Out-of-Scope {i+1}/{len(out_of_scope_cases)}] Querying: '{query}'"
            )
            ...
            try:
                ...
                is_refused = (
                    any(kw in answer.lower() for kw in refusal_keywords) or not contexts
                )
    ...
    try:
        ...
        correct_refusals = sum(
            1 for r in out_of_scope_results if r["is_correct_refusal"]
        )
        ...
        logger.info(
            f"Evaluation complete. Saved to docs/ragas_results.md. Abstention: {abstention_accuracy:.4f}"
        )
