...
def main():
    ...
    for doc in documents:
        try:
            ...
            logger.error(
                f"Failed to process or embed document '{doc['title']}': {str(e)}"
            )
    ...
    if points:
        logger.info(
            f"Upserting {len(points)} vector points to Qdrant in batches of {BATCH_SIZE}..."
        )
        for i in range(0, len(points), BATCH_SIZE):
            ...
            try:
                ...
                logger.error(
                    f"Qdrant connection failure or batch upsert error on batch {i // BATCH_SIZE + 1}: {str(e)}"
                )
                sys.exit(1)
    else:
