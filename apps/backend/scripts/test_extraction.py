...
class ExtractedEntity(BaseModel):
    name: str = Field(
        description="Formal name of the entity, e.g., 'High-Pressure Valve V-202'"
    )
    type: str = Field(
        description="Must be one of: concept, procedure, regulation, maintenance_log, drawing"
    )
    ...
    equipment_tags: List[str] = Field(
        default=[],
        description="List of standard equipment IDs, e.g., ['V-202', 'P-101']",
    )
    regulatory_references: List[str] = Field(
        default=[], description="List of referenced standards, e.g., ['OSHA 1910.119']"
    )
    linked_concepts: List[str] = Field(
        default=[],
        description="Titles/names of other entities mentioned in this text to create markdown links to",
    )
...
def main():
    ...
    for filename in sorted(files):
        ...
        try:
            ...
            status = "PASS"
            logger.info(
                f"Successfully extracted {entities_count} entities from {filename}"
            )
    ...
    print("=" * 80)
    print(
        f"{'File Name':<40} | {'Status':<6} | {'Entities':<8} | {'Time (s)':<8} | {'Details / Error':<20}"
    )
    print("-" * 100)
    for res in results:
        ...
        if len(details) > 30:
            details = details[:27] + "..."
        print(
            f"{res['file'][:40]:<40} | {res['status']:<6} | {res['entities']:<8} | {res['time']:<8.2f} | {details}"
        )
    print("=" * 80 + "\n")
